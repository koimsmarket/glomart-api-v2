...
async function syncLearnedCategoryKeywords(lease){
 const c=lease&&lease.client;if(!c)return {applied:false,reason:'lease_missing'};
 ...
 for(const group of groups){
  ...
  const canonical=S(dm.rows[0]&&dm.rows[0].keyword_canonical)||original;
 ...
 if(merged&&merged!==S(row.keyword))await c.query('UPDATE gm_category SET keyword=$2,updated_at=now() WHERE category_id=$1',[row.category_id,merged]);
 ...
 log('KEYWORD_SYNC',{category_id:row.category_id,before:S(row.keyword),after:merged,mappings,product_updated:productUpdated});
 return {applied:!!mappings.length,category_id:row.category_id,keyword:merged,mappings,product_updated:productUpdated};
}
