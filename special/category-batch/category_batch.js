...
router.get('/api/special/category-batch/control',async(req,res)=>{const m=auth(req,res);if(!m)return;try{const p=pool(req);await ensureControl(p);res.json({ok:true,version:'GM_CATEGORY_BATCH_SPECIAL_V019',control});}catch(e){log('CONTROL_READ_ERROR',{member:m,error:S(e&&e.message||e)});res.status(500).json({ok:false,error:'CONTROL_READ_FAILED'});}});
...
router.post('/api/special/category-batch/next',async(req,res)=>{const m=auth(req,res);if(!m)return;const p=pool(req),deviceId=S(req.body&&req.body.device_id);if(!deviceId)return res.status(400).json({ok:false,error:'device_id required'});try{await ensureControl(p);}catch(e){log('CONTROL_READ_ERROR',{member:m,stage:'next',error:S(e&&e.message||e)});return res.status(500).json({ok:false,error:'CONTROL_READ_FAILED'});}const bd=batchDate(req.body&&req.body.batch_date||control.batch_date);if(control.mode!=='RUN')return res.json({ok:true,state:control.mode,category:null,batch_date:bd});if(leases.has(deviceId)){const l=leases.get(deviceId),r=await refreshLeaseCategory(l);return res.json({ok:true,state:'LEASED',batch_date:bd,category:categoryPayload(r)});}
