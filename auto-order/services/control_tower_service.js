...
async function retryFailedWork(pool, input){
  ...
  try{
    ...
    const status=upper(before.work_status);

    /* Retry is intentionally idempotent.
       The control-tower list can be stale for a moment after the first click.
       If another request already restored the same work to READY/PENDING,
       do not report a false retry failure. */
    ...
    if(status==='RUNNING' || status==='LOCKED' || status==='CLAIMED'){
      throw new Error('work is already active: '+status);
    }

    if(status!=='FAILED'){
      throw new Error('work cannot be retried from status: '+status);
    }
    ...
    await db.query(`
      INSERT INTO gm_auto_order_log(
        ...
        $1,$2,'MANUAL_RETRY_FAILED',$3,'READY',$4,$5,$6,$7::jsonb,now()
      )
    `,[
      work.auto_order_no,
      work.work_id,
      ...
      JSON.stringify({
        previous_error_code:before.error_code||'',
        previous_error_message:before.error_message||''
      })
    ]);
