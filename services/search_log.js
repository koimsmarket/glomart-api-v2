...
module.exports = function installSearchLogService(deps){
  ...
  const handleSearchLog = async (req,res)=>{
    try{
      ...
      const deviceLang=cleanText(p.device_lang||p.deviceLang||(old&&old.device_lang)||'').slice(0,35);
      const translationSource=cleanText(p.translation_source||p.translationSource||(old&&old.translation_source)||'INPUT').slice(0,30);
      ...
      if(!old){
        ...
        saved=await dbQuery(`UPDATE gm_search_log SET keyword_original=$2,keyword_normalized=$3,keyword_canonical=$4,lang_code=$5,ui_lang_code=$5,keyword_lang_code=$6,country_code=$7,member_country_code=$8,category_code=$9,category_no=$10,category_name=$11,gmkr_result_count=$12,cpkr_result_count=$13,alkr_result_count=$14,smartfit_result_count=$15,db_insert_count=$16,queue_send_count=$17,cache_used=$18,member_id=$19,guest_key=$20,device_type=$21,id_search_count=$22,id_keyword_count=$23,guest_search_count=$24,guest_keyword_count=$25,merged_search_count=$26,merged_keyword_count=$27,merged_detail_count=$28,device_lang=$29,translation_source=$30,updated_at=now() WHERE search_event_id=$1 RETURNING *`,[eventId,original,normalized,canonical,uiLang,keywordLang,row.country_code,row.member_country_code,categoryCode,categoryNo,categoryName,counts.gmkr,counts.cpkr,counts.alkr,counts.smartfit,dbInsert,queueSend,row.cache_used,memberId,guestKey,dev,lateIdSearch,lateIdKeyword,lateGuestSearch,lateGuestKeyword,lateMergedSearch,lateMergedKeyword,lateMergedDetail,deviceLang,translationSource]);
      }
