...
async function processSmartfitMessageSend(client, payload){
  ...
  const batchSize=Math.max(10,Math.min(1000,Number(process.env.SMARTFIT_MESSAGE_BATCH_SIZE||100)));
  ...
  const rows=await client.query(`SELECT * FROM gm_smartfit_message_receiver
    ...
    ORDER BY message_no LIMIT ${batchSize} FOR UPDATE SKIP LOCKED`,[templateId,serialNo]);
  let sent=0;
  ...
  const totals=(await client.query(`SELECT COUNT(*)::int AS target_count,
      COUNT(*) FILTER (WHERE send_status IN ('SENT','READ'))::int AS sent_count,
      COUNT(*) FILTER (WHERE send_status='FAILED')::int AS failed_count,
      COUNT(*) FILTER (WHERE send_status IN ('QUEUED','QUEUED_NIGHT','PROCESSING'))::int AS pending_count
    FROM gm_smartfit_message_receiver WHERE template_id=$1 AND serial_no=$2`,[templateId,serialNo])).rows[0]||{};
  return {template_id:templateId,serial_no:serialNo,batch_size:batchSize,batch_sent:sent,remain,target_count:Number(totals.target_count||0),sent_count:Number(totals.sent_count||0),failed_count:Number(totals.failed_count||0),pending_count:Number(totals.pending_count||0)};
}
