...
module.exports = function createEventService(deps){
  ...
  async function applyBasketAdd(clientOrPool,row){
    ...
    try{
      ...
      const r=await client.query(`
        WITH target AS (
          SELECT p.product_uid,0 AS priority
          FROM gm_product p
          WHERE ($1::text<>'' AND p.product_uid=$1::text)
             OR ($2::text<>'' AND $3::text<>'' AND p.mall_code=$2::text AND p.pi_ii_vi=$3::text)
          UNION ALL
          SELECT p.product_uid,1 AS priority
          FROM gm_product_option o
          JOIN gm_product p ON p.mall_code=o.mall_code AND p.product_id=o.product_id
          WHERE $2::text<>'' AND $3::text<>'' AND o.mall_code=$2::text AND o.pi_ii_vi=$3::text
        ),
        picked AS (
          SELECT product_uid FROM target ORDER BY priority,product_uid LIMIT 1
        )
        UPDATE gm_product p
        SET cart_count=COALESCE(p.cart_count,0)+1,last_cart_at=now(),updated_at=now()
        FROM picked
        WHERE p.product_uid=picked.product_uid
        RETURNING p.product_uid,p.cart_count,p.cp_fix_code,p.cp_selected_code,p.category_code
      `,[uid,mall,pi]);
      const p=r.rows[0];
  ...
  async function upsertSalesPeriods(client,order,item){
    ...
    async function upProduct(table,col,val){ if(!(await tableExists(table))) return; await client.query(`INSERT INTO ${table} (${col},product_uid,pi_ii_vi,mall_code,product_name,category_no,category_code,sales_qty,sales_amount,purchase_amount,gross_profit,margin_rate,first_order_at,last_order_at,updated_at) VALUES ($1::text,$2::text,$3::text,$4::text,$5::text,$6::text,$6::text,$7::numeric,$8::numeric,$9::numeric,$10::numeric,CASE WHEN $8::numeric>0 THEN ROUND(($10::numeric/$8::numeric)*100,4) ELSE 0 END,now(),now(),now()) ON CONFLICT (${col},product_uid) DO UPDATE SET product_name=EXCLUDED.product_name,category_no=EXCLUDED.category_no,category_code=EXCLUDED.category_code,sales_qty=${table}.sales_qty+EXCLUDED.sales_qty,sales_amount=${table}.sales_amount+EXCLUDED.sales_amount,purchase_amount=${table}.purchase_amount+EXCLUDED.purchase_amount,gross_profit=(${table}.sales_amount+EXCLUDED.sales_amount)-(${table}.purchase_amount+EXCLUDED.purchase_amount),margin_rate=CASE WHEN (${table}.sales_amount+EXCLUDED.sales_amount)>0 THEN ROUND((((${table}.sales_amount+EXCLUDED.sales_amount)-(${table}.purchase_amount+EXCLUDED.purchase_amount))/(${table}.sales_amount+EXCLUDED.sales_amount))*100,4) ELSE 0 END,last_order_at=now(),updated_at=now()`,[val,uid,pi,mall,pname,cat,qty,sales,purchase,gross]); }
    async function upCategory(table,col,val){ if(!cat||!(await tableExists(table))) return; await client.query(`INSERT INTO ${table} (${col},category_no,category_code,category_name,sales_qty,sales_amount,purchase_amount,gross_profit,margin_rate,first_order_at,last_order_at,updated_at) VALUES ($1::text,$2::text,$2::text,'',$3::numeric,$4::numeric,$5::numeric,$6::numeric,CASE WHEN $4::numeric>0 THEN ROUND(($6::numeric/$4::numeric)*100,4) ELSE 0 END,now(),now(),now()) ON CONFLICT (${col},category_no) DO UPDATE SET sales_qty=${table}.sales_qty+EXCLUDED.sales_qty,sales_amount=${table}.sales_amount+EXCLUDED.sales_amount,purchase_amount=${table}.purchase_amount+EXCLUDED.purchase_amount,gross_profit=(${table}.sales_amount+EXCLUDED.sales_amount)-(${table}.purchase_amount+EXCLUDED.purchase_amount),margin_rate=CASE WHEN (${table}.sales_amount+EXCLUDED.sales_amount)>0 THEN ROUND((((${table}.sales_amount+EXCLUDED.sales_amount)-(${table}.purchase_amount+EXCLUDED.purchase_amount))/(${table}.sales_amount+EXCLUDED.sales_amount))*100,4) ELSE 0 END,last_order_at=now(),updated_at=now()`,[val,cat,qty,sales,purchase,gross]); }
