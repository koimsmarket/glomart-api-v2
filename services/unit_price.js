...
async function recalcProductUnitByUid(db,uid){
  ...
  for(const o of or.rows){
    options++;
    const ox=withRuleSource(calculate({ruleQty:rule.unit_rule_qty,ruleUnit:rule.unit_rule_unit,price:productPrice(o),texts:[o.option_name,p.product_name,p.mall_product_name]}),ruleSource);
...
async function recalcCategory(db,{gmCode='',all=false,apply=false}){
  ...
  while(true){
    ...
    for(const p of products){
      ...
      const x=withRuleSource(calculate({ruleQty:p.unit_rule_qty,ruleUnit:p.unit_rule_unit,price:productPrice(p),unitPriceText:p.unit_price_text,texts:[p.product_name,p.mall_product_name,JSON.stringify(p.option_json||'')]}),source);
      ...
      for(const o of options){
        out.options++;
        const ox=withRuleSource(calculate({ruleQty:p.unit_rule_qty,ruleUnit:p.unit_rule_unit,price:productPrice(o),texts:[o.option_name,p.product_name,p.mall_product_name]}),source);
...
module.exports={calculate,calculateMixed,recalcProductUnitByUid,recalcCategory,normUnit,glomartCodes,firstGlomartCode,findRuleByGmCode,findFirstValidRule};
