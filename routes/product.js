    ...
    try{
      await pool.query(`
        ...
        SET status=$2,
            ...
            result_json=$4::jsonb
        WHERE queue_id=$1
      `, [r.rows[0] && r.rows[0].queue_id, queueStatusAfterInline, inlineError, JSON.stringify({ chunk_index:toInt(p.chunk_index||p.chunkIndex,0), chunk_total:toInt(p.chunk_total||p.chunkTotal,0), mall_code:mallCode, saved:inlineSaved, skipped:inlineSkipped, audit:saveAudit, option_audit:optionAudit, sample:inlineResults.slice(0,10), errors:inlineResults.filter(x=>x && !x.ok).slice(0,30) })]);
