...
const VERSION = 'GM_ORDER_HISTORY_ROUTE_V004';
function clean(v){ return String(v == null ? '' : v).trim(); }
function db(req){ return req.app.locals.db || req.app.locals.pool; }
function statusFor(error){
  const m = clean(error && error.message || error);
  if(['member_id_required','order_no_required','action_required','unsupported_action'].includes(m)) return 400;
  if(m === 'order_not_found') return 404;
  if(['action_not_allowed_for_current_status','cafe24_action_must_use_cafe24_bridge','mixed_order_requires_item_selection'].includes(m)) return 409;
  return 500;
}
