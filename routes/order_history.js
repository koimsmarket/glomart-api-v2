...
router.get('/api/gm/order-history/list', async (req, res) => {
  ...
  try{
    ...
    console.log('[GM_ORDER_HISTORY_LIST_OK]', JSON.stringify({
      member_id:memberId,
      page:result.page,
      total:result.total,
      returned_orders:result.orders.length,
      stats:result.stats
    }));
    return res.json({ ok:true, version:VERSION, ...result });
  }catch(error){
    console.error('[GM_ORDER_HISTORY_LIST_FAIL]', JSON.stringify({member_id:memberId,error:String(error && error.message || error)}));
