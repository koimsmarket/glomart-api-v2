'use strict';
// GM_BUILDER_IMAGE_HNSW_REPRESENTATIVE_V019_SINGLE_SNAPSHOT_STATUS
...
async function status(db){
  ...
  return {...s,total_vector:total,candidate_vector:N(v.candidate_vector),current_done:current,excluded_run0:run0,unprocessed:Math.max(0,total-current-run0),representatives:N(m.representatives),max_representative_no:N(m.max_representative_no)};
}
router.get('/api/gm/builder/image-hnsw/representative/status',async(req,res)=>{const db=dbFrom(req);try{ok(res,await status(db));}catch(e){fail(res,500,'representative status failed',{detail:S(e&&e.message||e)});}});
router.get('/api/gm/builder/image-hnsw/representative/stats',async(req,res)=>{const db=dbFrom(req);try{const s=await settings(db);const r=await db.query(`SELECT representative_no,representative_puid,run_no,member_count,avg_similarity,min_similarity,max_similarity,updated_at FROM gm_image_vector_representative_stat WHERE run_no=$1 ORDER BY member_count DESC,representative_no LIMIT 500`,[s.run_no]);ok(res,{run_no:s.run_no,items:r.rows});}catch(e){fail(res,500,'representative stats failed',{detail:S(e&&e.message||e)});}});
