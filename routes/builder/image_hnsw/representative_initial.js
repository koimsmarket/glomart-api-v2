'use strict';
// GM_BUILDER_IMAGE_HNSW_REPRESENTATIVE_INITIAL_V019_SINGLE_SNAPSHOT_REBUILD
...
function previewLog(stage,data){
  try{console.log(`[GM_IMAGE_HNSW_REPRESENTATIVE_PREVIEW_V014] ${stage} ${JSON.stringify(data||{})}`);}catch(_){console.log(`[GM_IMAGE_HNSW_REPRESENTATIVE_PREVIEW_V014] ${stage}`);}
}
function initialLog(stage,data){
  try{console.log(`[GM_IMAGE_VECTOR_REPRESENTATIVE_INITIAL_V019_SINGLE_SNAPSHOT] ${stage} ${JSON.stringify(data||{})}`);}catch(_){console.log(`[GM_IMAGE_VECTOR_REPRESENTATIVE_INITIAL_V019_SINGLE_SNAPSHOT] ${stage}`);}
}
...
async function resetRepresentativeData(db,runNo){
  runNo=Math.trunc(Number(runNo));
  if(!(runNo>=1))throw new Error('재생성 RUN은 1 이상의 정수여야 합니다.');
  const c=await db.connect();
  try{
    await c.query('BEGIN');
    // Single-snapshot policy: every full rebuild replaces the previous test/operation result.
    // CSV files are the archive for discarded test RUNs.
    const stat=await c.query('DELETE FROM gm_image_vector_representative_stat');
    const map=await c.query('DELETE FROM gm_image_vector_representative_map');
    await c.query('COMMIT');
...
async function maxRepresentativeNo(db,runNo){const r=await db.query('SELECT COALESCE(MAX(representative_no),0)::bigint AS n FROM gm_image_vector_representative_map WHERE run_no=$1',[runNo]);return Number(r.rows[0]&&r.rows[0].n||0);}
...
router.get('/api/gm/builder/image-hnsw/representative/initial/export',async(req,res)=>{
  const db=dbFrom(req);
  try{
...
router.post('/api/gm/builder/image-hnsw/representative/initial/run',async(req,res)=>{
  const db=dbFrom(req);
  ...
  try{
    ...
    const reset=await resetRepresentativeData(db,st.run_no);
    initialLog('INITIAL_SNAPSHOT_RESET',{...reset,threshold:st.threshold});
    job.phase='QUEUED';
