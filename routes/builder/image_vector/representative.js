'use strict';
// GM_BUILDER_IMAGE_VECTOR_REPRESENTATIVE_V018_RUN_SCOPED_STATUS
...
async function status(db){
  ...
  const [vq,mq]=await Promise.all([
    ...
    db.query(`SELECT
      ...
      COUNT(DISTINCT run_no) FILTER (WHERE run_no>0 AND run_no<>$1)::int AS previous_run_count,
      COUNT(*) FILTER (WHERE run_no>0 AND run_no<>$1)::int AS previous_run_rows,
      ...
      FROM gm_image_vector_representative_map_run`,[s.run_no])
  ]);
  const v=vq.rows[0]||{},m=mq.rows[0]||{};const total=N(v.total_vector),current=N(m.current_done),run0=N(m.excluded_run0);
  return {...s,total_vector:total,candidate_vector:N(v.candidate_vector),current_done:current,previous_run_count:N(m.previous_run_count),previous_run_rows:N(m.previous_run_rows),excluded_run0:run0,unprocessed:Math.max(0,total-current-run0),representatives:N(m.representatives),max_representative_no:N(m.max_representative_no)};
}
