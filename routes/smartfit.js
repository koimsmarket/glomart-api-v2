...
router.get('/api/gm/smartfit/template/:template_id', async (req,res)=>{
  try{
    ...
    // Media table retired: images are resolved from deterministic R2 paths.
    ok(res,{ template, items:items.rows, media:[] });
...
router.get('/api/gm/smartfit/media/list', async (req,res)=>{
  // Deprecated. SmartFit images use deterministic R2 paths and image_count.
  ok(res,{ items:[], count:0, deprecated:true });
});

router.post('/api/gm/smartfit/media/save', async (req,res)=>{
  fail(res,410,'media table retired; use SmartFit R2 image APIs');
});
...
router.post('/api/gm/smartfit/comment/add', async (req,res)=>{
  try{
    ...
    const imageCount=Math.max(0, Math.min(3, i(b.image_count || b.imageCount, 0)));
    const r=await pool.query(`INSERT INTO gm_smartfit_comment (template_id,member_id,parent_id,rating,body,image_count,is_creator_reply)
      VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`, [templateId, member, b.parent_id || b.parentId || null, ratingRaw, s(b.body || b.comment || ''), imageCount, s(b.is_creator_reply || b.isCreatorReply || 'F') === 'T'?'T':'F']);
    if(ratingRaw !== null){
...
async function recordEventInternal(pool, body){
  ...
  // Raw event table is retired. Keep only the confirmed aggregate counters.
  await incrementCounters(pool, templateId, statType, amount);
  return { template_id:templateId, category_code:categoryCode, stat_type:statType, amount, lang_code:lang, stored:false };
}
...
router.get('/api/gm/smartfit/stat/monthly', async (req,res)=>{
  ok(res,{ items:[], count:0, postponed:true, note:'SmartFit event analytics is currently postponed' });
});
