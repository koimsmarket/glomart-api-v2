...
async function saveRepresentativeSettings(button,opts){
 ...
 try{
   const r=await fetch(`${API}/api/gm/builder/image-hnsw/representative/initial/settings`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({run_no:runNo,threshold})});
   ...
   if(!opts.silent){log({action:'image-hnsw.representative.settings',run_no:j.run_no,threshold:j.threshold});await loadRepresentativeStatus();}
   return j;
...
async function loadRepresentativeStatus(){
 ...
 try{
   const [sr,jr]=await Promise.all([
     fetch(`${API}/api/gm/builder/image-hnsw/representative/status?t=${Date.now()}`,{cache:'no-store'}),
     fetch(`${API}/api/gm/builder/image-hnsw/representative/initial/status?t=${Date.now()}`,{cache:'no-store'})
   ]);
...
async function fetchRepresentativeInitialStatus(){
 const r=await fetch(`${API}/api/gm/builder/image-hnsw/representative/initial/status?t=${Date.now()}`,{cache:'no-store'});
...
async function previewRepresentativeCategories(button){
 ...
 try{
   const r=await fetch(`${API}/api/gm/builder/image-hnsw/representative/initial/preview`,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
   const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.detail||j.error||`HTTP ${r.status}`);
   log({action:'image-hnsw.representative.preview',...j});
...
async function runRepresentativeBuilder(button){
 ...
 try{
   ...
   const r=await fetch(`${API}/api/gm/builder/image-hnsw/representative/initial/run`,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
   const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.detail||j.error||`HTTP ${r.status}`);
   log({action:'image-hnsw.representative.initial.run',...j});
